...
def adjust(value, name, im, isWindow, howAdjust):
    adjustedImage = im if isWindow == True else readImage(im)
    
    if howAdjust == "resize":
        if not value == None:
            adjustedImage = resize(value, adjustedImage, isWindow)
...
while True:
    ...
    select.geometry(str(800+width) + 'x400+200+230') 
...
#             selectBox.delete(s, s)
    ...
    while f < len(selectedImages):
        ...
        def adjustImage(name):
            ...
            setAdjust.geometry('450x50+580+230')
            ...
            def cropImage():                
                setCrop = Tk()
                setCrop.title("Set crop boundary color")
                setCrop.geometry('100x50+630+230')
                
                def getColor(c):
                    global cropColor
                    cropColor = askcolor(color = c, parent = setCrop, title = "Choose crop boundary color")
                    cropColor = ((255.99609375, 255.99609375, 255.99609375), '#ffffff') if cropColor == (None, None) else cropColor 
                    print(cropColor)
               
                boundary = Button(setCrop, text = "Set crop color", command = lambda : getColor(cropColor))
                boundary.pack()
                setCrop.mainloop()
            ...
            resizeButton = Button(setAdjust, text = "Resize", command = lambda : adjust(processValue(factor.get()), name, image, True, "resize"))
            ...
            cropButton = Button(setAdjust, text = "Crop", command = cropImage)
            cropButton.grid(column = 3, row = 1)
        ...
        pWidth = str(300 + len(o.name)**2)
        imageProcessing.geometry(pWidth + 'x50+500+230') 
        ...
        names = ["Adjust", "Invert", "RGB Multiplication", "Contrast", "Convolve", "Tone Mapping", "Bilateral Filter", "Warp Perspective", "Pixelate"]
        ...
        operations = [adjustImage, invertImage, setRGBMultiplicationValues, contrastImage, setConvolve, toneMapping, setBilateralFilter, warpPerspective, pixelate]
